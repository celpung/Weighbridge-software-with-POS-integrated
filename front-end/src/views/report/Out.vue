  ...
  <div class="container-fluid">
    <div class="row border m-3 p-3">
      <div class="row d-flex justify-content-end">
        <div class="col-auto">
          <button data-bs-toggle="modal" data-bs-target="#filterModalOut" class="btn btn-sm btn-basic">Filter</button>
        </div>
      </div>
      <h5 class="text-center">Laporan Penjualan</h5>
      <h6 class="text-center">Periode {{ sd_out }} s/d {{ ed_out }}</h6>
      <ul class="list-group list-group-flush">
        <li class="list-group-item" v-for="(sell, index) in selling" :key="index">
          <div @click="showData(sell.product)" class="d-flex justify-content-between" data-bs-toggle="modal" data-bs-target="#dataOut">
            <span>{{ sell.product }} ({{ new Intl.NumberFormat("id-ID").format(sell.totalNetto) }} Kg)</span>
            <span>{{ new Intl.NumberFormat("id-ID").format(sell.totalPrice) }} IDR</span>
          </div>
        </li>
    ...
    <div class="modal fade" id="filterModalOut" aria-hidden="true">
            ...
            <form id="filter-form">
              <div class="mb-2">
                ...
                <div class="input-group">
                  <input id="start_date" v-model="start_date_out" type="date" class="form-control shadow-none border-end-0" />
              ...
              <div class="mb-2">
                ...
                <div class="input-group">
                  <input id="end_date" v-model="end_date_out" type="date" class="form-control shadow-none border-end-0" />
                  <div class="input-group-text bg-white"><i class="fa fa-calendar" aria-hidden="true"></i></div>
                </div>
              </div>
            </form>
...
export default {
  ...
  methods: {
    getItem() {
      let model = {
        start: this.start_date_out,
        end: this.end_date_out,
      };
      ...
      var sellingData = this.calibrations.filter(function (value) {
        return value.product_status == "KELUAR" && value.total_price != "0";
      });

      var sellingList = [...new Set(sellingData.map((item) => item.product_name))];

      this.selling = [];
      (this.sumNetto = 0),
        (this.sumTotalPrice = 0),
        sellingList.forEach((element) => {
          var data = sellingData.filter(function (value) {
