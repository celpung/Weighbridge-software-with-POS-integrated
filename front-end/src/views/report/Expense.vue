  ...
  <div class="container-fluid">
    <div class="row">
      <div class="container-fluid d-flex justify-content-between align-items-center mb-3 mt-3">
        ...
        <h5 class="m-0 p-0"><u>Tabel Pengeluaran</u></h5>
        <div>
          <button @click="resetFilter()" class="btn btn-danger m-1">Reset</button>
          <button data-bs-toggle="modal" data-bs-target="#expenseModal" class="btn btn-success m-1">Filter</button>
        </div>
      </div>
      <div class="table-responsive">
        <table class="table text-center">
          ...
          <tbody>
            <tr v-for="(ex, index) in expense" :key="index">
              <td>{{ ex.date }}</td>
              <td>{{ ex.account }}</td>
              <td>{{ ex.info }}</td>
          ...
          <tfoot>
            <tr>
              ...
              <td>{{ new Intl.NumberFormat("id-ID").format(totalExpense) }}</td>
            </tr>
          </tfoot>
        </table>
      </div>
        ...
        <div class="modal-content">
          ...
          <form>
            ...
            <div class="modal-footer">
              ...
              <button type="button" @click="filterExpense()" data-bs-dismiss="modal" class="btn btn-primary">Filter</button>
            </div>
          </form>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getExpense() {
      ...
      this.expense = this.reports.filter(function (value) {
    ...
    counting() {
      this.totalExpense = this.expense.reduce(function (accumulator, item) {
        return accumulator + parseInt(item.ammount);
      }, 0);
    },

    setFilter() {
      this.accountList = [...new Set(this.expense.map((item) => item.account))];
    },
    ...
    filterExpense() {
      ...
      if (model.start == "" || model.start == null || model.start == undefined) {
        var itemStart = this.expense.at(0);
      ...
      this.expense = this.reports.filter(function (value) {
        ...
        return value.type == "expense" && dates >= startDate && dates <= endDate && acc;
      });
      this.counting();
    },
